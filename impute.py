...
# copy raw -> process on first run
raw_files = [f for f in os.listdir(RAW_DIR) if f.endswith('.json')]
if not raw_files:
    st.sidebar.error(f"ไม่พบไฟล์ JSON ต้นฉบับใน `{RAW_DIR}`")
    st.stop()

for f in raw_files:
    p_path = os.path.join(PROCESS_DIR, f)
    if not os.path.exists(p_path):
        shutil.copy2(os.path.join(RAW_DIR, f), p_path)
...
st.sidebar.markdown("---")
view_mode = st.sidebar.radio("โหมดการทำงาน:", ["แก้ไขข้อมูล (Process)", "ดูต้นฉบับ (Raw)"])
...
if is_raw_mode:
    st.warning("โหมดดูต้นฉบับ (Raw): ปุ่มบันทึกจะถูกล็อกการใช้งาน", icon="🔒")
else:
    st.info("โหมดแก้ไข (Process): การแก้ไขทั้งหมดจะบันทึกลงโฟลเดอร์ Process", icon="💾")

st.title(f"{selected_amphoe} / {selected_filename}")

# tambon / unit selectors
    ...
    if st.button("เปิด PDF", type="secondary"):
        ...
        if not os.path.exists(unit_dir):
            ...
            try:
                ...
                target_pdfs = []
                for pdf in pdfs:
                    if is_ss_mode and "บช" not in pdf:
                        target_pdfs.append(pdf)
                    elif not is_ss_mode and "บช" in pdf:
                        target_pdfs.append(pdf)
                if not target_pdfs:
                    target_pdfs = pdfs
                ...
                if target_pdfs:
                    ...
                    st.warning(f"ไม่พบไฟล์ PDF ในโฟลเดอร์: {unit_dir}")
            except Exception as e:
                st.error(f"เกิดข้อผิดพลาดในการเปิด PDF: {e}")
...
# handle missing OCR table
if len(votes) == 0 and not is_raw_mode:
    st.warning("ไม่พบข้อมูลตารางในไฟล์ OCR กรุณาเลือกวิธีจัดการข้อมูลด้านล่าง:")
...
with st.form("edit_form"):
    ...
    txt_note = st.text_input(
        "หมายเหตุ:",
        value=d.get("manual_note", ""),
        placeholder="เช่น ลายมืออ่านยาก, มีรอยขีดฆ่า",
        disabled=is_raw_mode,
    )

    st.markdown("---")
    st.markdown("##### คะแนนแต่ละพรรค/ผู้สมัคร (เรียงตามเบอร์)")
    ...
    for i, kid in enumerate(sorted(votes.keys(), key=lambda x: int(x))):
        ...
        if not is_ss_mode:
            try:
                disp_name = PARTY_NAMES[int(kid) - 1]
            except IndexError:
                disp_name = ""
        else:
...
if submit_btn and not is_raw_mode:
    ...
    bs.update({
        "ballots_valid":   edit_valid,
        "ballots_spoiled": edit_spoiled,
        "ballots_no_vote": edit_novote,
        "ballots_used":    edit_used,
        "ballots_alloc":   edit_alloc,
        "ballots_remain":  edit_remain,
    })
